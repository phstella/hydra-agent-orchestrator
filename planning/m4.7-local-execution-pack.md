...
|---|---|---|---|---|
| M4.7.1 | Cockpit Shell and Desktop Layout | DONE | agent | CockpitShell.tsx: 3-col layout (left rail 56px, top strip 44px, center flex, right rail 320px). data-testid coverage. Smoke tests 1, 19. |
| M4.7.2 | Center Race Config Convergence | DONE | agent | CockpitCenter.tsx: race config panel in cockpit center. Adapter selection, workspace display, prompt, start race button. Smoke test 4. |
| M4.7.3 | Right Leaderboard Card Model | DONE | agent | LeaderboardRail.tsx: per-agent cards with lifecycle badge, elapsed time, score snapshot, mergeability hint, failure messages. Smoke test 19. |
| M4.7.4 | Focused Live Terminal Panel | DONE | agent | CockpitCenter.tsx: terminal bound to selectedAgent from leaderboard. Focus switch via LeaderboardRail selection. Smoke test 20. |
| M4.7.5 | Inline Intervention in Cockpit | DONE | agent | CockpitCenter.tsx: InputComposer wired inline below terminal for running agents. Error feedback via interventionError state. |
| M4.7.6 | Completion Summary and Review Bridge | DONE | agent | CompletionSummary.tsx: winner card, score, mergeability, other candidates. Open Diff Review CTA navigates to review view. Smoke test 21. |
| M4.7.7 | Error and Degradation UX | DONE | agent | Errors shown in: leaderboard cards (failure text), cockpit center (race error banner), top strip (status dot), terminal (transport errors). |
| M4.7.8 | QA Hardening and Regression Gate | DONE | agent | 31 smoke tests pass (27 migrated + 4 new cockpit tests). cargo test workspace (271 tests), cargo clippy (clean), hydra-app (49 tests), npm run lint (pass), npm run test:smoke (31 pass). |
